...
async function fetchDriveStreamWithApiKey(fileId, apiKey, options = {}) {
  ...
  if (!response.ok || !response.body) {
    throw driveError(`Drive API media request failed with status ${response.status}.`, response.status, 'DRIVE_API_ERROR');
  }
...
async function fetchDriveStream(fileId, options = {}) {
  ...
  if (apiKey) {
    try {
      return await fetchDriveStreamWithApiKey(fileId, apiKey, options);
    } catch (error) {
      // Fall back to public-link flow if API key path fails.
    }
  }
