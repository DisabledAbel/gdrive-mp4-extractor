...
function ensureResourceKey(url, resourceKey) {
  const nextUrl = new URL(url, 'https://drive.google.com');
...
function extractFormActionUrl(html, fileId, resourceKey) {
  ...
  const actionUrl = new URL(formMatch[1], 'https://drive.google.com');
...
async function fetchDriveStream(fileId, options = {}) {
  ...
  for (let i = 0; i < 9; i += 1) {
    ...
    if (contentType.includes('text/html')) {
      ...
      if (directLink) {
        url = ensureResourceKey(directLink.replace(/&amp;/g, '&'), resourceKey);
        continue;
      }

      const downloadUrl = extractGoogleDownloadUrl(html);
      if (downloadUrl) {
        url = ensureResourceKey(downloadUrl, resourceKey);
        continue;
      }
